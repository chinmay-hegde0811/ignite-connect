    ...
    const capturePayment = async () => {
        ...
        try {
            ...
            if (response.statusCode === 200) {
                ...
                if (result.action === 'PARTIAL_CAPTURED' || result.status === 'CAPTURED') {
                    ...
                    <div className="transaction-form-wrapper">
                        <div className='transaction-form'>
                            <div className="input-block">
                                <p>{formatMessage(messages.amountToCapture)}</p>
                                <input
                                    type="number"
                                    name="captureAmount"
                                    value={captureAmount}
                            ...
                            <div className="transaction-button" onClick={capturePayment}>
                                {loading ? <LoadingSpinner /> : <PrimaryButton label={formatMessage(messages.capture)} />}
                            </div>
                        </div>
                        <div className="transaction-details">
                            {formatMessage(messages.amountCapturedAlready)} <b>{currencyCode} {alreadyCapturedAmount}</b>
...
CaptureAmount.propTypes = {
    ...
    paymentProviderId: PropTypes.string,
